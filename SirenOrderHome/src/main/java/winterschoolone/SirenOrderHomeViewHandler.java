...
public class SirenOrderHomeViewHandler {
    ...
    public void whenPayed_then_CREATE_1 (@Payload Payed payed) {
        try {
            if (payed.isMe()) {
            	...
            	sirenOrderHome.setCouponQty(payed.getCouponQty());
            	sirenOrderHome.setStampQty(payed.getStampQty());
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenIssued_then_UPDATE_3(@Payload Issued issued) {
        try {
            if (issued.isMe()) {
                // view 객체 조회
            	List<SirenOrderHome> sirenOrderHomeList = sirenOrderHomeRepository.findByOrderId(issued.getOrderId());
                for(SirenOrderHome sirenOrderHome : sirenOrderHomeList){
                	sirenOrderHome.setShopId(issued.getId());
                	sirenOrderHome.setStatus("Coupon issued");
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenUsed_then_DELETE_2(@Payload Used used) {
        try {
            if (used.isMe()) {
            	List<SirenOrderHome> sirenOrderHomeList = sirenOrderHomeRepository.findByOrderId(used.getOrderId());
                for(SirenOrderHome sirenOrderHome : sirenOrderHomeList){
                	sirenOrderHome.setShopId(used.getId());
                	sirenOrderHome.setStatus("Coupon used");
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    // view 레파지 토리에 save
                	sirenOrderHomeRepository.save(sirenOrderHome);
                }
